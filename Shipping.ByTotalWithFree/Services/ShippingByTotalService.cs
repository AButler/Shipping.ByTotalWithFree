...
namespace Nop.Plugin.Shipping.ByTotalWithFree.Services {
  /// <summary>
      ...
      if ( shippingByTotalRecordZip != null &&
         ( shippingByTotalRecordZip.Contains( ',' ) || // multiple zip entries
          ...
          shippingByTotalRecordZip.Contains( '*' ) ) )  // starts with wildcard
            {
        var allZips = shippingByTotalRecordZip.Split( new[] { ',' }, StringSplitOptions.RemoveEmptyEntries );
        ...
        foreach ( var entry in allZips ) {
          ...
          if ( zipEntry.Contains( ':' ) ) {
            var zipRanges = zipEntry.Split( new[] { ':' }, StringSplitOptions.RemoveEmptyEntries );
